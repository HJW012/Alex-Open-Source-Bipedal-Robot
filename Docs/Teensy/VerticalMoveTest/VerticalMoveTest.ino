...
#define motor_1_ID 11 // Parent ID
#define motor_2_ID 12 // Child ID
...
float p_in[] = {0.0f, 0.0f}; //Position
float v_in[] = {0.0f, 0.0f}; //Speed
float kp_in[] = {100.0f, 100.0f}; //Stiffness
float kd_in[] = {1.0f, 1.0f}; //Damper
float t_in[] = {0.0f, 0.0f}; //Torque

// Measured Values - received from motor
float p_out[] = {0.0f, 0.0f};
float v_out[] = {0.0f, 0.0f};
float t_out[] = {0.0f, 0.0f};

bool inMotorMode[] = {false, false};
...
void loop() {
  ...
  Serial.println("FootZ: " + String(goalZ));
  handleButtons();

  // Do something to datya
  getJoystickData();
  kneeIkine("left");
...
void handleButtons() {
  if (!digitalRead(motorModeButton) && !debounce1) {
    if (motorsZeroed) {
      toggleMotorMode(motor_1_ID);
      toggleMotorMode(motor_2_ID);
    } else {
  ...
  if (!digitalRead(zeroMotorsButton) && !debounce2) {
    if (!inMotorMode[0] && !inMotorMode[1]) {
      zeroMotor(motor_1_ID);
      zeroMotor(motor_2_ID);
       motorsZeroed = true;
    }
...
bool kneeIkine(String prefix) {
  if (prefix == "left") {
    ...
    convertToTmotorPose(q1, q2);
  }
}

bool convertToTmotorPose(double q1, double q2) {
  // Assuming motors have been zeroed at the defined starting poses
  // Motors rotate clockwise positive
  double p_in_0 = startingQ2 - q2;
  double p_in_1 = startingQ1 - q1 - p_in_0;
